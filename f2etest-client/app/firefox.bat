...
taskkill /fi "username eq %username%" /s %computername% /u %username% /f -IM firefox.exe >nul
start /MAX "" "C:\Program Files (x86)\Mozilla Firefox\firefox.exe" %url%
set isWeb=1
if "%1" equ "" (
	set isWeb=0
)
start "" curl "http://f2etest.xxxx.com/applog?userid=%USERNAME%&appid=firefox&isweb=%isWeb%"